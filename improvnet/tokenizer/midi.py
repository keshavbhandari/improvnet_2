...
class MidiDict:
    ...
    def get_program_to_instrument(cls) -> dict[int, str]:
        ...
        for instrument, programs in INSTRUMENT_TO_PROGRAM.items():
            for program in programs:
                PROGRAM_TO_INSTRUMENT[program] = instrument
    ...
    def resolve_overlaps(self) -> "MidiDict":
        ...
        for channel, msgs_by_pitch in note_msgs_c.items():
            for pitch, msgs in msgs_by_pitch.items():
                ...
                prev_off_tick = -1
                prev_on_tick = -1  # <-- ADD THIS
                for idx, msg in enumerate(msgs):
                    on_tick = msg["data"]["start"]
                    off_tick = msg["data"]["end"]
                    
                    # MODIFY THIS CONDITION
                    if prev_off_tick > on_tick and prev_on_tick != on_tick:
                        # Adjust end of previous (idx - 1) msg to remove overlap
                        msgs[idx - 1]["data"]["end"] = on_tick
                        
                    prev_off_tick = off_tick
                    prev_on_tick = on_tick  # <-- ADD THIS
        ...
        # Note messages
            ...
            if (message.note, message.channel) in last_note_on:
                ...
                notes_to_close = [
                    (start_tick, velocity)
                    for start_tick, velocity in open_notes
                    if start_tick <= end_tick  # <--- FIX: This now includes zero-duration notes
                ]
                notes_to_keep = [
                    (start_tick, velocity)
                    for start_tick, velocity in open_notes
                    if start_tick > end_tick   # <--- FIX: This handles edge cases
                ]
                ...
                if len(notes_to_keep) > 0:
                    # If we have notes to keep, update the buffer
                    last_note_on[(message.note, message.channel)] = (
                        notes_to_keep
                    )
                else:
                    # Otherwise, all notes were closed, so clear the buffer
                    del last_note_on[(message.note, message.channel)]

    # --- FIX FOR HANGING NOTE_ON MESSAGES (e.g., drum files) ---
    # After processing all messages, check for any un-closed notes.
    # This is common in drum tracks where note_off is omitted.
    for (note, channel), open_notes in last_note_on.items():
        for start_tick, velocity in open_notes:
            # Create a NoteMessage with zero duration (end_tick = start_tick)
            note_msgs.append(
                {
                    "type": "note",
                    "data": {
                        "pitch": note,
                        "start": start_tick,
                        "end": start_tick,  # Set end time equal to start time
                        "velocity": velocity,
                    },
                    "tick": start_tick,
                    "channel": channel,
...
if __name__ == "__main__":
    ...
    midi_filepath = "/keshav/improvnet_2/improvnet/inference/debussy-clair-de-lune_original.mid"
    # Read MIDI file into MidiDict
    midi_dict_obj = MidiDict.from_midi(midi_filepath)
    print(midi_dict_obj.note_msgs[0:20])