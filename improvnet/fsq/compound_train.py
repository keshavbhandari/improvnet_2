...
from improvnet.fsq.compound_config import (
    ...
    GRAD_CLIP, VAL_EVERY, SAVE_DIR, VAL_LOG_FILE, 
    RESUME_TRAINING, JSONL_FILES, RUN_NAME
)
...
class MIDISequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.processor.get_random_segment_from_data(tokens, self.seq_len)
...
def train(n_steps: int = N_STEPS):
    ...
    running_loss, running_acc, log_steps = 0.0, 0.0, 0
    model.train()
    ...
    while step < n_steps:
        ...
        for x in train_loader:
            ...
            with amp_context(AMP_DTYPE):
                # x shape is now just [B, T, 5]
                logits_list, z_q, indices = model(x)
                raw_loss = model.module.loss(logits_list, x)
            ...
            if is_main_process and (step % LOG_EVERY == 0) and log_steps > 0:
                ...
                total_codes = flat.shape[0]
                
                print(f"  Step {step:>6} | Loss: {avg_loss:>6.4f} | Acc: {avg_acc*100:>6.2f}% | Codes: {unique:>5,}/{total_codes:,}")

                writer.add_scalar('Train/Loss', avg_loss, step)
                writer.add_scalar('Train/Accuracy', avg_acc * 100, step)
                writer.add_scalar('Train/Codebook_Usage', unique / total_codes, step)
            ...
            if step > 0 and step % VAL_EVERY == 0:
                val_loss, val_acc = evaluate_validation(model, val_loader, local_rank, device, max_batches=100)
                if is_main_process:
...
@torch.no_grad()
def evaluate_validation(model, val_loader, local_rank, device, max_batches: int = 100): # Lowered default to 100
    """
    Evaluates the model on the validation set. 
    Synchronizes metrics across all GPUs if running in DDP.
    Limits evaluation to max_batches to speed up the validation phase.
    """
    model.eval()
    total_loss = 0.0
    total_acc = 0.0
    ...
    # Handle both DDP wrapped and unwrapped models gracefully
    model_obj = model.module if hasattr(model, 'module') else model

    # --- THE FIX: DDP-Safe TQDM Progress Bar ---
    is_main_process = (local_rank == 0)
    
    # Calculate the actual total for the progress bar (whichever is smaller)
    total_steps = min(len(val_loader), max_batches)
    
    # Only wrap the dataloader in tqdm if this is the main GPU
    ...
    for x in iterator:
        ...
        with torch.amp.autocast('cuda', enabled=(AMP_DTYPE == torch.float16)):
            # x is perfectly shaped: [Batch, Seq_Len, 5]
            logits_list, _, _ = model(x)
            
            # Calculate loss and accuracy using class methods
            loss = model_obj.loss(logits_list, x)
    ...
    # Calculate local averages for this specific GPU
    local_avg_loss = total_loss / max(1, steps)
    local_avg_acc = total_acc / max(1, steps)
    
    # If running in Distributed mode, synchronize metrics across all GPUs
    if torch.distributed.is_initialized():
        metrics = torch.tensor([local_avg_loss, local_avg_acc], device=device)
        # Sum the averages from all GPUs
        torch.distributed.all_reduce(metrics, op=torch.distributed.ReduceOp.SUM)
        # Divide by the number of GPUs to get the true global average
