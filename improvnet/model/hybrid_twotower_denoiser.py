...
class AdaLN(nn.Module):
    """Adaptive Layer Norm to ingest specific Diffusion Timesteps per draft."""
...
class HybridGroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, seq_pos, context_kv=None, draft_size=0):
        ...
        # Connect to the Broadcasted Anchor Point from Tower A
...
class TwoTowerDenoiser(nn.Module):
    ...
    def forward(self, noisy_target, timestep, seq_offset=0, context_kv_cache=None, draft_size=0):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Tower A only outputs valid (K,V) caches at its Flash Attention layers (indices 3, 7, 11...).
            # We broadcast Anchor 3 to Tower B layers 0-3. Anchor 7 to layers 4-7.
            if context_kv_cache is not None:
                anchor_idx = min(((i // 4) * 4) + 3, len(context_kv_cache) - 1)
                layer_context_kv = context_kv_cache[anchor_idx]
            else:
                layer_context_kv = None
